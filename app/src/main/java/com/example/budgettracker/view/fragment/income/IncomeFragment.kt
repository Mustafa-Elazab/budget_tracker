package com.example.budgettracker.view.fragment.income
...
@AndroidEntryPoint
class IncomeFragment : BaseFragment<FragmentIncomeBinding, HomeViewModel>() {
    ...
    private fun initViews() = with(binding) {
        appBar.toolbar.setTitle(R.string.income)
        ...
        transactionAdapter.setOnItemClickListener {
            ...
            findNavController().navigate(
                R.id.action_incomeFragment_to_detailBudgetFragment,
                bundle
            )
        }
    ...
    private fun observeFilter() = with(binding) {
        lifecycleScope.launchWhenCreated {
            viewModel.transactionFilter.collect { filter ->
                when (filter) {
                    "Income" -> {
                        totalIncome.tvTotalBalance.text =
    ...
    private fun onTotalTransactionLoaded(transaction: List<Transaction>) = with(binding) {
        ...
        val income = totalIncomeBalance.sumOf {
            it.amount
        }

        totalIncome.tvTotalBalanceNumber.text =
            income.toString()
    }
